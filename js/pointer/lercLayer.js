var LercLayer = L.GridLayer.extend({
  createTile: function(coords, done) {
    ...
    var url =
      "https://elevation3d.arcgis.com/arcgis/rest/services/WorldElevation3D/" +
      "Terrain3D/ImageServer/tile/" +
      coords.z +
      "/" +
      coords.y +
      "/" +
  ...
  draw: function(tile) {
    ...
    var f = 256 / (max - min);
    var pv = 0;
    const getPixelValue = (pixels, index, f) =>
      (pixels[index + Math.floor(index / width)] - min) * f;
    for (var i = 0; i < width * height; i++) {
      // Skip the last pixel in each input line
      pv = getPixelValue(pixels, i, f);
      data[i * 4] = pv;
      data[i * 4 + 1] = pv;
      data[i * 4 + 2] = pv;
